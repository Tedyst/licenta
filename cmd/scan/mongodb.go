...
var scanMongodbCmd = &cobra.Command{
	Use:   "mongodb [connection string]",
	Short: "Scan a MongoDB database",
	Long:  `This command allows you to scan a MongoDB database manually. It does not update the database, so the results will not be visible in the web interface. The database connection string is not required, but is recommended for the bruteforce module. If a bruteforce is successful, the database will store that result in order to avoid repeating the bruteforce.`,
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		sc, err := mongodb.NewScanner(context.Background(), client)
		if err != nil {
			return err
		}
		...
		for _, user := range users {
			...
			slog.Info("verifypassword", "err", err, "ok", ok)
		}
...
func init() {
	scanMongodbCmd.Flags().String("database", "", "Database connection string")
	scanCmd.AddCommand(scanMongodbCmd)
}
